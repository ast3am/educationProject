...
import (
	...
	"errors"
	"fmt"
	"github.com/ast3am/educationProject/internal/user"
	"github.com/ast3am/educationProject/pkg/logging"
	"strconv"
)

type repository struct {
	storage map[string]*user.UserModel
	id      int
	logger  *logging.Logger
}

func NewRepository(ctx context.Context, rep map[string]*user.UserModel, logger *logging.Logger) user.Repository {
	return &repository{
		storage: rep,
		logger:  logger,
	}
...
func (r *repository) Create(ctx context.Context, user *user.UserModel) error {
...
func (r *repository) FindFriend(ctx context.Context, id string) (ufriends []*user.UserModel, err error) {
	//проверка на существование
	_, ok := r.storage[id]
	if !ok {
		err := errors.New("Пользователь " + id + " не найден\n")
